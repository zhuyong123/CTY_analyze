...
<BODY>
	...
	<div style="text-align: center;">
		<p>
			<a href="#" target="_blank">SnapMatch管理系统${status}</a>
		</p>
	</div>
	<script type="text/javascript">
		function login(){
			...
			//document.getElementById("form1").action="${ctx}/login.do";
			//document.getElementById("form1").submit();
			$.ajax({                                                                      
				  type:'post',                                                            
				  url: '${ctx}/login.do?method=post',                                                               
				  dataType: 'json',
				  async:false,      
				 data: {userName:$('#userName').val(),password:$('#password').val()},                                                 
				 success: function(data){                                                
						if(data.status==1){
							window.location.href="${ctx}/manage.do";
						}else{
							alert("用户名或密码不对！");
						}
	             },
